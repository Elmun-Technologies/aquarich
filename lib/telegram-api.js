...
function getWebAppUrl() {
  var url = (process.env.WEBAPP_URL || process.env.VERCEL_URL || "").replace(/\/$/, "");
  if (!url) return "";
...
function buildStartMessage(firstName) {
  ...
  return (
    "👋 Salom" + name + "!\n\n" +
    "<b>Zilol Suv</b> — toza suv filtrlari.\n\n" +
...
function getWebAppUrls(startParam) {
  ...
  return {
    b2c: base + "/index.html?utm_source=telegram" + src,
    b2b: base + "/b2b.html?utm_source=telegram" + src,
  };
}
